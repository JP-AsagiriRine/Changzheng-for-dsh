...
## Feature list (v1.1 / runtime package pkg-12)

| Area | Content |
| --- | --- |
| Theme gating | Styles and UI take effect **only while the `changzheng` theme is active**; switching back to light/dark fully restores the native product UI, other themes are unaffected; the settings switch row is always present |
...
1. **Theme switching is not persisted**: state is in-memory for the dynamic
   ...
   restored — click 启用 again (the plugin itself stays running for the
   session). Switching back to light/dark unloads all Changzheng styling and
   UI, keeping only the settings switch row.
